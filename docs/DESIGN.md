...
The prototype's era (15 years, one town) is generated fresh at run start — town, hosts, relationships, one prime prophecy with three conditions (a local, era-sized win in the same shape as the campaign's Prime), and N loose prophecies (dial, default 3). Human lifespans are clamped to the ceiling in code. Two animal hosts always exist: a raven and a rat; the town's one sealed location admits only them. Comprehension traded for access, as the rat at the Ascension established.
...
| 5 | **The briefing shows the prime openly.** Right for a systems test; the campaign let the Prime arrive as discovered treasure. Should the real game hide the prime until first contact? | Deferred until systems testing is done. One-line change either way. |
