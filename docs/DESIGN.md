...
| 7 | **Decay tempo — CONFIRMED TOO FAST in first live playtests** (designer, 2026-07-26: "prophecies are being closed / moving too quickly"). The root cause is likely not the number but the definition of contact: any fact sharing one tag counts, so a host doing their *ordinary work* near a prophecy's subject burns it — inhabit the ferryman for two routine scenes and the ferry prophecy is gone. The arrival-scene directive (host mid-task in ordinary work) makes this worse: the game deliberately opens scenes inside the subjects. Incidental contact and meaningful contact are currently the same thing. Candidate refinements, in rising order of change: (a) raise the contacts dial to 3–4 — available now; (b) require ≥2 shared tags for a contact, so single-tag brushes are noise; (c) count at most one contact per prophecy per in-world year, so tempo runs on the calendar, not the scene count — "left too long" starts meaning *time*, which is what the original threat always implied; (d) storyteller marks facts significant/incidental and only significant facts brush. | Open. The dial mitigates today; the definition of contact is the real decision, and it is the designer's. |
